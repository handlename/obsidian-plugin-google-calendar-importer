...
#### FR-011: テンプレート未設定時の出力省略
- **優先度**: 必須
- **説明**: テンプレート設定が空の場合、該当する種類の予定を出力しない
- **根拠**: ユーザーが通常予定または終日予定のいずれか一方のみを出力したい場合に対応するため
- **依存**: FR-004, FR-010
- **検証基準**:
  - 通常予定のテンプレートが空文字列または空白のみの場合、通常予定は出力されない
  - 終日予定のテンプレートが空文字列または空白のみの場合、終日予定は出力されない
  - 両方のテンプレートが空の場合、何も出力されない
  - 設定画面の説明文に「Leave empty to omit ... events from output」の記載がある
  - 空のテンプレート設定もバリデーションエラーとならない
